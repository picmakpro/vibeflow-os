...
| `v2.31.1` | 2026-07-25 | **Version-file alignment**: `software-architecture` and `kpi-analyst` shipped their Windows-portability fixes in v2.29.0 (CHANGELOG and `module.json` both bumped) but their `VERSION` files stayed behind — the engine reads `VERSION`, so the registry reported a stale number. No user was denied the fix (the changes live in `scripts/`, which the governance resync recopies), but the version registry now tells the truth (software-architecture v1.5.1, kpi-analyst v1.0.1) |
| `v2.31.0` | 2026-07-25 | **Fine-grained intent routing**: three routing levels (trigger-dense skill descriptions with cross-referenced counter-examples, a global verb-precedence rule, an on-demand exhaustive routing doctrine covering all 65 engine skills). 19 new `/vf-*` verbs — dev-orchestrator goes from 14 to 31, design-orchestrator gains `/vf-sketch`. The agent's table no longer names any internal target: it routes to a verb, the verb knows its target. Also: third-party project mentions purged repo-wide, T5/T11 test axes scoped to their own module, rule-loading semantics corrected, description template applied to the three conductor verbs (dev-orchestrator v1.8.1, design-orchestrator v1.1.0, conductor v1.12.2) |
