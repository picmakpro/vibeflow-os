...
| 43-02-T1 | 43-02 | 3 | FABR-07 | T-43-11, T-43-12 | écart déclaration/prose dans les deux sens, avertissement (exit 0), jamais refus | unit (bash) + mutation | test-check-skills.sh T22-T29 (T26 en T26a-T26h ; T26b à T26h fixent la règle Q-PORTEE, tranchée le 2026-09-26 : un marqueur seul en prose → rien, deux distincts en prose → dérive, un en titre → dérive, même marqueur répété → rien, marqueurs dans un bloc de code → ni détectés ni comptés, sens frontmatter → corps sous la même règle), constantes `DERIVE_PROSE_MIN_DISTINCTS = 2` et `DERIVE_TITRE_MIN = 1`, MUT-DR1/DR2 (DR2 par sa trace) ; `PREMIER-COMMIT-G2` | ❌ W0 | ⬜ pending |
...
| 43-04-T2 | 43-04 | 2 | FABR-09 | T-43-30 | décision bootstrap déjà tranchée (ratchet-socle, décision déléguée par Willy au head (/vf-decide), AskUserQuestion session principale, 2026-09-26) consignée verbatim avec ses motifs et la mesure du jour — plus un checkpoint | grep | `DECISION-Q1-FIN` sans ligne MANQUE, BASE-ABSENTE ni BASELINE-TOUCHEE | n/a (SUMMARY) | ⬜ pending |
| 43-04-T3 | 43-04 | 2 | FABR-09 | T-43-30, T-43-31, T-43-34 | socle du bootstrap borné par la ligne `@bootstrap:socle` (hausse → DEPASSEMENT-BOOTSTRAP, rc 1 armé ; baisse → rc 0 sans jeton de dépassement), plafond ADR-029 signalé par AU-DESSUS-PLAFOND-ADR029, non bloquant ; ligne citée, seule ajoutée, dans le même commit atomique que la métrique, signalée à Samuel | unit (bash) + mutation + G-1 + rejeu par commit | BOOT-1..5, MUT-11, MUT-12 ; `REEL-BOOT rc=0 mesure=<n> ligne=<n>` ; `G1 rc=0/3` ; `BASELINE-DIFF ajoutees=1 bootstrap=1` ; `REJEU <sha> rc=0` pour chaque commit puis `REJEU-FIN` ; `SIGNALEMENT-FIN <sha7>` | ❌ W0 | ⬜ pending |
...
| 43-06-T1 | 43-06 | 4 | FABR-10 (c) + docs | T-43-50, T-43-51 | vf-calibrate à deux clés, conductor en mineure | grep + check-version-sync | `CONDUCTOR-VERSION-OK base=… attendu=… obtenu=…` (mineure prouvée contre la VERSION à B43) ; `COMPTES-FIN` ; `BOOTSTRAP-DOC` sans valeur nulle (ratchet-socle, `@bootstrap:socle`, AU-DESSUS-PLAFOND-ADR029 et citation dans 43-04-SUMMARY, README et CHANGELOG) | ✅ | ⬜ pending |
