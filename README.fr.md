...
| **[mobile-test-team](./plugin/mobile-test-team/)** | `1.3.1` | agents + rules | 🤖 Boucle autonome test→fix mobile : `vf-test-orchestrator` + workers cloisonnés par outils (`vf-test-runner` / `vf-app-fixer`, Pattern 12), pour que le mode autonome atteigne « l'app marche vraiment », pas juste des tests unitaires verts. Une règle path-scopée déclenche la doctrine de vérification réelle pendant le code. Requiert `mobile-test`. **Expérimental**. |
...
| **[validator](./plugin/validator/)** | `1.2.1` | agent-only | Agent `vibeflow-validator` : garant de l'alignement technique méthodo ↔ projets, en 5 phases (dont audit d'architecture des process). |
| **[consolidator](./plugin/consolidator/)** | `1.6.2` | skill + scripts | Consolidation de la mémoire structurée sur 4 piliers (indexation / archivage / fusion / promotion) + pilier mémoire vivante (couche `knowledge/` fichier-par-entrée, décroissance par demi-vie de catégorie, supersession non destructive) + registres fork-config. |
