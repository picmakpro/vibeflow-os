...
| Couverture amont | **MESURES EXACTES, CONCLUSION INVALIDÉE — voir ADR-069** | Re-mesure `awk`+`comm` sur les **91 workflows racine** (compte confirmé) : **7 connaissent** les workstreams, **45 codent en dur** `.planning/ROADMAP.md`/`STATE.md`/`phases`, dont **42 sans aucune conscience** (`execute-phase`, `execute-plan`, `plan-phase`, `discuss-phase`, `next`, `ship`, `pr-branch`, `quick`, `progress`, `complete-milestone`…). **Couverture réelle : 7/91 = 7,7 %.** — *Renvoi (2026-08-04, nœud 24-13) : les trois nombres sont exacts, le verdict « bien pire que 18 % » est INVALIDÉ. `ADR-069` a établi que 7/91 est le critère **K2** et que le ~18 % du ROADMAP est **K3** (17,6 % bruts / 16,5 % réels, un faux positif nommé) : trois critères, aucune mesure fausse.* |
