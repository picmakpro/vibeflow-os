...
| `check-agents.sh` (`isolation:` frontmatter) | `plugin/conductor/scripts/check-agents.sh:39,160,528-530` | **Objet distinct, à ne pas confondre** — `isolation: worktree` est une décision de dispatch séparée (issue #38), jamais dérivée de `scope[]` | `team-kernel.md:126-138` : « L'isolation est une décision de DISPATCH, jamais une propriété du worker » |
