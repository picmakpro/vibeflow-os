...
[![Version](https://img.shields.io/badge/version-2.33.0-2563eb)](./VERSION)
...
Today VibeFlow orchestrates **dev and design** through an **agentic model**: the `vibeflow-dev` agent detects intent and invokes the GSD toolchain directly, with a mission team for long runs. Other domains are built as **tailor-made labs** through the Lab Factory (`vf-new-lab` + `skill-creator`); the ready-made content / growth / business bundles are **in preparation**. Beyond orchestration, VibeFlow ships **governance** modules: software-architecture audits, infrastructure audits, memory consolidation, methodology-alignment validation — each enabled à la carte.
