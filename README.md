...
Today VibeFlow orchestrates **dev and design** through an **agentic model**: the `vibeflow-dev` agent detects intent and invokes the GSD toolchain directly, with a mission team for long runs. Other domains are built as **tailor-made labs** through the Lab Factory (`vf-new-lab` + `skill-creator`); the **content, growth and business-pilot bundles are available** (full teams on the team-kernel). Beyond orchestration, VibeFlow ships **governance** modules: software-architecture audits, infrastructure audits, memory consolidation, methodology-alignment validation — each enabled à la carte.
...
|--------|:----:|------|--------------|
| **[conductor](./plugin/conductor/)** | `1.14.1` | agent + skills + scripts + references | 🧭 The front door. Meta agent `vibeflow-conductor` (guardian): create/configure a lab in **any domain** (`vf-new-lab`, bundle-aware), install/verify/update, migrate on doctrine change (`vf-calibrate`), receive coherence escalations. Not always-on — config/audit/migration only. |
...
| **[validator](./plugin/validator/)** | `1.3.1` | agent-only | Agent `vibeflow-validator`: guardian of technical alignment between methodology and projects, in 5 phases (incl. process-architecture audit). |
| **[consolidator](./plugin/consolidator/)** | `1.8.0` | skill + scripts | Structured-memory consolidation across 4 pillars (indexing / archiving / merging / promotion) + living-memory layer (per-entry `knowledge/`, category half-life decay, non-destructive supersession) + fork-config registries. |
