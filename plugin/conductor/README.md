...
**Type** : `agent + skills + scripts + references` · **Version** : v1.14.5 · **Dépend de** : `planning-core`, `validator`, `skill-creator`.
...
| **Cloisonnement par tools** (P12) | juges sans Write/Edit, la plupart des workers sans Task, allowlist `Agent(...)` sur les managers, `vf-internal: true` | anti-triche vérifié par les suites de test des modules (contenu de l'allowlist) — `check-agents.sh` linte le frontmatter, pas le contenu de `tools:`. L'invariant « un seul manager actif » tient malgré l'exception (`vf-coder`/`vf-reviewer`/`vf-auditer` gardent un `Agent` non scopé) grâce au verrou de driver, qui referme aussi le chemin indirect — détail : `references/team-kernel.md` |
