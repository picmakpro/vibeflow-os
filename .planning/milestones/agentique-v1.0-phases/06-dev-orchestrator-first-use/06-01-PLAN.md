...
`wc -l` après édition.
  ...
  <verify>
    <automated>/bin/bash -c 'cd "$(git rev-parse --show-toplevel)" && A=dev-orchestrator/AGENT.md && n=$(wc -l < "$A" | tr -d " ") && test "$n" -le 250 && [ "$(command grep -v "^#" "$A" | command grep -ci "first-use\|premier usage")" -ge 1 ] && command grep -q "\.planning" "$A" && command grep -q "vf-init" "$A" && [ "$(command grep -v "^#" "$A" | command grep -ci "new-project")" -ge 1 ] && echo "OK-AGENT ${n}L"'</automated>
  </verify>
  <done>
