...
| Fichier | Résumé |
|---|---|
| [GSD-PIPELINE.md](GSD-PIPELINE.md) | Doctrine de pipeline GSD complète, chargée on-demand par `vibeflow-dev` / `vf-dev-manager` (règle 1%). |
...
| [docs-flow.md](docs-flow.md) | Source de vérité de la sortie documentaire : les quatre familles de docs que `vibeflow-dev` distingue (DOCF-01→04). |
...
| [ingestion-flow.md](ingestion-flow.md) | Doctrine du pont spec/plan → feuille de route : comment `vibeflow-dev` détecte une spec/plan à ingérer (BRDG-01/03). |
