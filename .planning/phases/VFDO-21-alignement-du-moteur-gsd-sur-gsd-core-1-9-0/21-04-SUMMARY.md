...
- **Tâches** : gate machine (2 invariants), ADR d'arbitrage, correctif de mise en forme de
  `.planning/STATE.md` + recalage du frontmatter.
...
- **Gate `check-state-integrity.sh`** (`plugin/conductor/scripts/`, module `conductor` v1.18.0) :
  ...
  (`--against`, défaut `HEAD`) et échoue si `completed_phases`/`completed_plans`/`current_phase`
  ont régressé **au sein d'un même jalon** (`milestone:` inchangé — un changement de jalon
  réinitialise légitimement ces compteurs, garde explicite). Vérifie en même temps que le corps ne
  porte jamais plus d'une ligne `^Phase:` (cause B). Suite dédiée 23 cas, dont 2 discriminations
  machine par comparaison directe de deux exécutions (garde de jalon, 1 vs 2 lignes `^Phase:`) sur
  le patron de `test-check-mission-invariants.sh` cas 5b. 4 mutants tués manuellement avant
  livraison (garde de régression, garde `^Phase:`, garde de jalon, garde de ref `--against`) — le
  script restauré a été vérifié identique par `diff` après chaque mutation.
...
- `bash plugin/conductor/scripts/check-state-integrity.sh` (repo réel, `--against HEAD~3`,
  l'état juste avant ce plan) → `EXIT: 0`, conforme.
- Gates de sortie rejoués sur le module `conductor` : 13 suites (dont
  `test-check-state-integrity.sh`, 23 cas), toutes vertes (0 KO cumulé) ; `check-version-sync.sh`
  vert hors les 2 lignes attendues et hors périmètre (compteur de suites racine) ;
  `check-mission-invariants.sh` → `EXIT: 3`, SAIN.
- Mutation du gate : 4 mutants appliqués, tous tués (6, 2, 1, 2 KO respectivement), script restauré
  identique par `diff` après chacun.
