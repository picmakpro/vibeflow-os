...
- **Plafond ADR-029 des agents du bundle relevé à 300 lignes** — `content/BUNDLE.md`, les trois
  blueprints d'agent (`campaign-analyst`, `channel-strategist`, `copywriter-sequences`) et la
  suite `test-growth-bundle.sh` (T2) énoncent et testent désormais ≤300L (au lieu de ≤250L).
...
- `vf-growth-manager.md` : la doctrine du verrou s'arrêtait à `acquired:false → ne dispatche pas,
  remonte à l'humain` sans jamais nommer de marche à suivre sur un lock périmé. Ajout d'une
...
### Modifié
- **Les 5 agents du bundle déclarent `effort:`** — `vf-growth-manager` **high** et
  `growth-quality-judge` **high** (pilotage et jugement) ; `channel-strategist`,
  `campaign-analyst` et `copywriter-sequences` **medium** (exécution). Barème par rôle repris des
...
## [v2.0.3] — 2026-07-31 (barrière d'écriture réelle de `growth-quality-judge`, Phase 20)

### Corrigé
- **`growth-quality-judge` porte `disallowedTools: Write, Edit`** : la barrière d'écriture était
...
- **`vf-growth-manager` cite le mécanisme réel** (au lieu du seul adjectif « read-only ») pour
