...
rc_of() { # <repo> <module> <kind> [extra args...] -> imprime le rc, capture stdout/stderr dans $LAST_OUT
  local r="$1" m="$2" k="$3"
  shift 3
  LAST_OUT="$(bash "$CMB" "$m" "$k" --root "$r" "$@" 2>&1)"
  return $?
}
...
rc_of "$R" m patch --main-ref main; rc4=$?
[ "$rc4" -eq 1 ] && v=ok || v=ko
...
rc_of "$R" m patch --main-ref main; rc5=$?
[ "$rc5" -eq 1 ] && v=ok || v=ko
...
rc_of "$R" m patch --main-ref main; rc6=$?
[ "$rc6" -eq 1 ] && v=ok || v=ko
...
rc_of "$R" m patch --main-ref main; rc7=$?
case "$LAST_OUT" in
