#!/usr/bin/env bash
# test-business-pilot-bundle.sh — Suite de vérification du module business-pilot-bundle
# (équipe business sur le team-kernel, matérialisation 2026-07-25). Transpose les 12 tests
# du content-bundle + 2 tests spécifiques métier (T13 human-gate d'envoi client, T14 zéro
# chiffre inventé).
#
...
#         passed|gaps_found|human_needed|blocked) dans le manager ET chaque worker/gate.
#   T8  — Validation humaine NON contournable : human_needed + « jamais » dans le manager ;
#         le finance REFUSE de facturer un livrable non validé ; le skill porte l'invariant.
#   T9  — Skill vf-business : description valide (déclencheur + portée) + aiguillage
#         geste simple vs mission (SEUIL_EQUIPE_BUSINESS → vf-business-manager).
...
#   T12 — Rubric du gate : /100 explicite + seuil 80 + critères éliminatoires (montant non
#         sourcé, hors périmètre vendu).
#   T13 — [spécifique] Human-gate d'ENVOI CLIENT non contournable : « jamais » + human_needed
#         chez manager, commercial ET finance ; « n'envoie/n'envoi » explicite chez les deux
#         workers qui rédigent des documents sortants ; l'invariant d'envoi dans le skill.
#   T14 — [spécifique] Zéro chiffre inventé : Iron Law (invent + sourc) chez le finance ET le
#         manager ; le gate rend le montant non sourcé éliminatoire.
#
...
MANAGER="$MOD/agents/vf-business-manager.md"
GATE="$MOD/agents/quality-gate-client.md"
COMMERCIAL="$MOD/agents/vf-business-commercial.md"
FINANCE="$MOD/agents/vf-business-finance.md"
WORKERS="vf-business-commercial vf-business-delivery vf-business-finance"
TEAM="vf-business-manager vf-business-commercial vf-business-delivery vf-business-finance quality-gate-client"
SKILL="$MOD/skills/vf-business/SKILL.md"

echo "== test-business-pilot-bundle (module: $MOD) =="
...
if [ -f "$SKILL" ]; then
  n=$(wc -l < "$SKILL" | tr -d ' ')
  [ "${n:-999}" -le 500 ] || { ko "T2 densité : vf-business/SKILL.md = ${n}L (>500)"; t2_ok=0; }
else
...
if [ -z "$CHECK_AGENTS" ]; then
  ...
  if bash "$CHECK_AGENTS" --strict --agents-dir="$MOD/agents" >/dev/null 2>&1; then
    ok "T3 check-agents : --strict vert sur les agents de l'équipe business (ADR-044)"
  else
...
# ---------------------------------------------------------------------------
# T4 — Gate read-only : pas de Write/Edit, vf-internal
# ---------------------------------------------------------------------------
t4_ok=1
if [ -f "$GATE" ]; then
  jtools=$("$GREP" '^tools:' "$GATE")
  echo "$jtools" | "$GREP" -qw 'Write' && { ko "T4 gate : Write présent dans tools"; t4_ok=0; }
  echo "$jtools" | "$GREP" -qw 'Edit'  && { ko "T4 gate : Edit présent dans tools"; t4_ok=0; }
  echo "$jtools" | "$GREP" -qE 'Task|Agent' && { ko "T4 gate : Task/Agent présent dans tools"; t4_ok=0; }
  "$GREP" -q '^vf-internal: true' "$GATE" || { ko "T4 gate : vf-internal manquant"; t4_ok=0; }
  "$GREP" -qi 'ne modifie' "$GATE" || { ko "T4 gate : consigne read-only absente du corps"; t4_ok=0; }
else
  ko "T4 gate : $GATE introuvable"; t4_ok=0
fi
[ "$t4_ok" -eq 1 ] && ok "T4 gate : read-only machine-enforced (tools sans Write/Edit/Task) + vf-internal"
...
# ---------------------------------------------------------------------------
# T8 — Validation humaine non contournable (ADR-031)
# ---------------------------------------------------------------------------
t8_ok=1
if [ -f "$MANAGER" ]; then
...
if [ -f "$FINANCE" ]; then
  "$GREP" -qi 'REFUSE' "$FINANCE" || { ko "T8 humain : le finance ne refuse pas explicitement un livrable non validé"; t8_ok=0; }
  "$GREP" -qi 'validation humaine' "$FINANCE" || { ko "T8 humain : gate validation humaine absent du finance"; t8_ok=0; }
fi
if [ -f "$SKILL" ]; then
  "$GREP" -qi 'jamais' "$SKILL" && "$GREP" -qi 'validation humaine' "$SKILL" \
    || { ko "T8 humain : invariant validation humaine absent du skill"; t8_ok=0; }
fi
[ "$t8_ok" -eq 1 ] && ok "T8 humain : validation humaine non contournable (manager + finance + skill)"

# ---------------------------------------------------------------------------
# T9 — Skill vf-business : description + aiguillage
# ---------------------------------------------------------------------------
t9_ok=1
if [ -f "$SKILL" ]; then
  ...
  "$GREP" -q 'SEUIL_EQUIPE_BUSINESS' "$SKILL" || { ko "T9 skill : seuil d'aiguillage absent"; t9_ok=0; }
  "$GREP" -q 'vf-business-manager' "$SKILL" || { ko "T9 skill : route mission → vf-business-manager absente"; t9_ok=0; }
else
...
# ---------------------------------------------------------------------------
# T12 — Rubric du gate : /100, seuil 80, critères éliminatoires métier
# ---------------------------------------------------------------------------
t12_ok=1
if [ -f "$GATE" ]; then
  "$GREP" -q '/100' "$GATE" || { ko "T12 rubric : /100 absent du gate"; t12_ok=0; }
  "$GREP" -q '80' "$GATE" || { ko "T12 rubric : seuil 80 absent du gate"; t12_ok=0; }
  "$GREP" -qi 'éliminatoire' "$GATE" || { ko "T12 rubric : critère éliminatoire absent du gate"; t12_ok=0; }
  "$GREP" -qi 'sourc' "$GATE" || { ko "T12 rubric : critère de sourcing des montants absent du gate"; t12_ok=0; }
  "$GREP" -qi 'périmètre vendu' "$GATE" || { ko "T12 rubric : critère « périmètre vendu » absent du gate"; t12_ok=0; }
else
  ko "T12 rubric : $GATE introuvable"; t12_ok=0
fi
[ "$t12_ok" -eq 1 ] && ok "T12 rubric : /100 + seuil 80 + éliminatoires (montant non sourcé, hors périmètre vendu)"

# ---------------------------------------------------------------------------
# T13 — [métier] Human-gate d'ENVOI CLIENT non contournable
# ---------------------------------------------------------------------------
t13_ok=1
for pair in "manager:$MANAGER" "commercial:$COMMERCIAL" "finance:$FINANCE"; do
  label="${pair%%:*}"; f="${pair#*:}"
  if [ ! -f "$f" ]; then ko "T13 envoi : agent $label introuvable"; t13_ok=0; continue; fi
  "$GREP" -qi 'jamais' "$f" || { ko "T13 envoi : « jamais » absent chez $label"; t13_ok=0; }
  "$GREP" -q 'human_needed' "$f" || { ko "T13 envoi : human_needed absent chez $label"; t13_ok=0; }
done
for pair in "commercial:$COMMERCIAL" "finance:$FINANCE"; do
  label="${pair%%:*}"; f="${pair#*:}"
  [ -f "$f" ] || continue
  "$GREP" -qiE "n'envoie jamais|N'ENVOIE JAMAIS|jamais d'envoi|aucun envoi" "$f" \
    || { ko "T13 envoi : interdiction d'envoi explicite absente chez $label"; t13_ok=0; }
done
if [ -f "$SKILL" ]; then
  "$GREP" -qiE "jamais d'envoi|aucun envoi|n'envoie" "$SKILL" \
    || { ko "T13 envoi : invariant d'envoi client absent du skill"; t13_ok=0; }
fi
[ "$t13_ok" -eq 1 ] && ok "T13 envoi : human-gate d'envoi client non contournable (manager + commercial + finance + skill)"

# ---------------------------------------------------------------------------
# T14 — [métier] Zéro chiffre inventé (Iron Law finance + gate + manager)
# ---------------------------------------------------------------------------
t14_ok=1
if [ -f "$FINANCE" ]; then
  "$GREP" -qi 'invent' "$FINANCE" || { ko "T14 chiffres : Iron Law (invent) absente du finance"; t14_ok=0; }
  "$GREP" -qi 'sourc' "$FINANCE" || { ko "T14 chiffres : exigence de source absente du finance"; t14_ok=0; }
  "$GREP" -qi 'kpi-analyst' "$FINANCE" || { ko "T14 chiffres : adossement kpi-analyst absent du finance"; t14_ok=0; }
else
  ko "T14 chiffres : $FINANCE introuvable"; t14_ok=0
fi
if [ -f "$GATE" ]; then
  "$GREP" -qi 'montant non sourcé' "$GATE" || { ko "T14 chiffres : « montant non sourcé » absent du gate"; t14_ok=0; }
fi
if [ -f "$MANAGER" ]; then
  "$GREP" -qi 'invent' "$MANAGER" || { ko "T14 chiffres : Iron Law (invent) absente du manager"; t14_ok=0; }
fi
