...
6. **Instanciation des agents** — pour CHAQUE blueprint de `content/agents/`, crée
   `.claude/agents/<nom>.md` ≤300L : recopie le frontmatter cible **intégralement**, sans en
   sélectionner les champs — une liste dupliquée dérive (gates `check-blueprints.sh` en amont et
   `check-agents.sh` sur l'agent posé, ADR-044), la
