...
#        références NE DOIT PAS suffire — d'où l'extraction de section. Vérifie aussi que
#        le digest du manager porte les interdits du lab issus du CLAUDE.md.
# ---------------------------------------------------------------------------
t13_ok=1
t13_extract_methode() {
...
if [ -f "$JUDGE" ]; then
  ...
  if [ -z "$methode" ]; then
    ...
    "$GREP" -v "l'outil Read, le \`CLAUDE.md\` du lab" "$JUDGE" > "$t13_tmp"
...
if [ -f "$MANAGER" ]; then
  digest_section=$(awk '/^## Digest de mission/{f=1;next} f&&/^## /{exit} f' "$MANAGER")
  echo "$digest_section" | "$GREP" -qi 'interdits' \
    || { ko "T13 D-08 : section « Digest de mission » du manager ne mentionne pas les interdits du lab"; t13_ok=0; }
  echo "$digest_section" | "$GREP" -q 'CLAUDE.md' \
    || { ko "T13 D-08 : section « Digest de mission » du manager ne cite pas le CLAUDE.md comme source"; t13_ok=0; }
fi
[ "$t13_ok" -eq 1 ] && ok "T13 D-08 : lecture explicite du CLAUDE.md ancrée dans le DÉROULÉ du juge + digest du manager portant les interdits du lab"
