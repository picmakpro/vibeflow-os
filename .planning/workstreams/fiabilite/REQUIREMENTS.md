...
| LEDG-01 | Phase 18 | Done — plans 18-01/18-02/18-03 |
| LEDG-02 | Phase 18 | Done — plans 18-01/18-02/18-03 |
| LEDG-03 | Phase 30 | Ouverte le 2026-08-15 — https://github.com/open-gsd/gsd-core/issues/3556 — deadline amont 2026-10-26 |
| BUDG-01 | Phase 25 | Complete — 2026-09-16, plan 25-04 : 31 fichiers mesurés et publiés (`.planning/instruction-budget-baselines.tsv`, conductor v1.37.1) |
| BUDG-02 | Phase 25 | Complete — 2026-09-16, plan 25-04 : ratchet armé (sentinelle `.planning/.instruction-budget-armed`), gate rend 0 armé |
| BUDG-04 | Phase 40.1 | Complete — 2026-09-17, plan 40.1-14 : recensement versionné final rc 0 (0 ligne), contrôles négatifs verts (fixtures 11/11, clone réel 7/7), rejeu CI gates+tests vert, dix bumps dérivés vérifiés |
| BUDG-05 | Phase 40.1 | Complete — 2026-09-17, plan 40.1-14 : bascules CI prouvées par mutants (plan 40.1-03), invariants de phase (M, I1-I4) verts, rejeu gates (10 étapes) et tests (79 suites/0 échec) verts |
...
- [x] **BUDG-01**: Le budget d'instructions par fichier d'agent distribué est mesuré et publié (métrique tranchée au cadrage de la phase) — **close le 2026-09-16** (plan 25-04) : `check-instruction-budget.sh` mesure 31 fichiers (lignes du fichier, instructions du body, D-01 bis), valeurs publiées dans `.planning/instruction-budget-baselines.tsv` (3641 lignes / 487 instructions) et au CHANGELOG conductor v1.37.1
...
- [ ] **PROT-01**: Rulesets de branche et de tags posés et prouvés par un refus réel. **NON COCHÉ**
  ...
  **Mise à jour (audit 2026-09-24)** : les deux rulesets (`main`, `tags-v`) sont **actifs depuis le
  2026-09-23 19h56** (mesuré) — « posés » est donc acquis. Ce qui manque encore est la preuve par
  **refus réel**. Arbitrage de Willy (WhatsApp, 2026-09-24) : la PR #104 coche sur une mesure de
  configuration (lecture des rulesets), ce n'est PAS un refus ; **la PR #100 en est un** — passée
  de `BEHIND` à `BLOCKED` après remise à jour, bloquée par la revue code owner malgré le bypass
  disponible. Elle servira de preuve de refus réel une fois mergée. Reste décochée jusque-là.
...
- [x] **ADPT-06**: La fermeture du canal d'injection est prouvée en **RÉPÉTITIONS** (≥ 3 runs sur le banc témoin, marqueur attendu **0/N**), jamais en un run — l'injection mesurée est **non déterministe (2/3)** : un run propre ne prouve rien. Si un seul run sur N ressort le marqueur, la porte n'est PAS fermée. **Limite (audit 2026-09-24)** : la preuve `0/N` ne couvre QUE le canal `skills`/`AGENTS.md` (le `0/5` mesuré). **Le canal `hooks`/`plugins` du dépôt jugé n'a jamais eu ses propres répétitions** (voir `3b7de24`) — reliquat inscrit au `BACKLOG.md` du compartiment `fiabilite`, pas un travail refait ici.
...
- [x] **PART-02**: La **non-composabilité du pointeur avec ADR-064** est **tranchée par la
  ...
  `CLAUDE_CODE_SESSION_ID` (`CLAUDE_CODE_CHILD_SESSION=1`), un manager qui dispatche des workers
...
- [x] **PART-04**: Le **filet de détection de divergence** existe et est **bruyant** — signature
  ...
  **ET câblé sur le job CI `gates`** (arbitrage Samuel, 2026-09-10 — le hook local seul serait muet
  par construction sur les PR fusionnées côté GitHub, le chemin de merge dominant de ce dépôt ;
  ADR-059, « sûr mais inerte », précédent Phase 35 non répété). **Mutation rouge prouvée** (D-13)
  sur les trois variantes de split-brain de la recherche §8 (orphan, rootonly, roadmap) : le gate
...
- [x] **PART-05**: Le comportement de `/gsd-pr-branch` sur un dépôt partitionné est traité **sur la
  ...
  ne matche pas `STRUCTURAL_RE`, ancré `^\.planning/ROADMAP\.md$`, `pr-branch.md:255`) — un commit
  qui ne touche QUE la feuille de route d'un workstream est **EXCLU dans les deux modes**
  (`pr_strict` vrai ou faux), invisible dans tout rapport. Les trois conséquences (i FAUSSE À
  MOITIÉ / ii VRAIE / iii VRAIE) sont gravées avec leur verdict. Choix explicite tranché et écrit,
  **jamais implicite** : **coût assumé, daté** (amendement d'ADR-069) — pas de correctif local de
...
- [x] **PART-09**: VF passe **`--ws` explicitement** sur ses propres appels `gsd_run` (agents
  ...
  `GSD_WORKSTREAM` exporté dans l'environnement partagé** (39-03-PLAN.md Tâche 1, corrigé le même
  jour — sinon la preuve confond « le flag fonctionne » et « l'export fonctionne ») — aucun
  workflow amont n'est réécrit (D-05). Une **veille datée sur un ÉVÉNEMENT** (prochaine publication
  `gsd-core`, qui distribuera `#4455`/`#4456`/`#4225`, fermés le 7-8 septembre 2026 mais non
...
### Transverse
- [ ] **QUAL-01**: Tout nouveau gate du milestone naît avec ses trois issues (PASS / FAIL / imparsable BRUYANT) et sa mutation rouge prouvée — mesuré satisfait sur la Phase 41 le 2026-09-18 (vingt mutants tués sur les trois gardes plus neuf sur les deux outils de phase, total vingt-neuf, rc=0 sur les cinq suites), **puis RÉGRESSÉ** : la partition du planning (PR #94, D-02, 2026-09-23) a inséré deux niveaux entre `.planning/` et `phases/`, cassant la résolution de racine des deux outils de phase (`check-trace-arbitrage.sh`, `check-aucune-fermeture.sh` — comptage de `../` littéral). Mesuré avant réparation : `test-check-trace-arbitrage.sh` rendait rc=2 des DEUX côtés de MUT-5 (NON-VERIFIABLE, incapacité totale à distinguer un détecteur large d'un détecteur resserré), preuve devenue non-mesurable sans que personne ne s'en aperçoive (aucun des deux outils n'est câblé dans un job CI). **Réparé le 2026-09-24** (résolution par `git rev-parse --show-toplevel`, jamais un compte de niveaux) : `test-check-aucune-fermeture.sh` repasse à 15/15 ; `test-check-trace-arbitrage.sh` passe de rc=2 (NON-VERIFIABLE) à rc=1 — **20/21, pas 21/21** : MUT-1/2/3/4/6 tués, MUT-5 mécaniquement correct (le détecteur élargi trouve bien une dérive) mais NON TUÉ, car le contrôle sur la plage réelle actuelle trouve une dérive RÉELLE et préexistante (plusieurs commits fusionnés depuis la dernière mesure du registre citent deux arbitrages distincts et conformes dans le même message — `CITATIONS-MULTIPLES`, refusé PAR CONCEPTION par ce détecteur). Total mesuré : **28 mutants tués sur 29**, pas 29/29. Reste NON SATISFAIT tant que `BASE-TRACE-ARBITRAGE` (registre de Willy, hors périmètre de cet audit) n'est pas avancé ou que la dérive n'est pas autrement résorbée — ne pas cocher avant cette mesure.
