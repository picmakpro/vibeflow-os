...
key-decisions:
  ...
  - "vf-dev-manager.md ferme le volet OPEN de D-10 pour VF : nouveau paragraphe exigeant un compartiment nommé explicitement par mandat + un GSD_SESSION_KEY distinct par mandat concurrent. Édition net line-neutral (paragraphe Filet de repli D-09 condensé en compensation) — fichier à 250 lignes (mesuré après une correction ultérieure), marge nulle sur le plafond ADR-029 (250). Machine-vérifié : plugin/dev-orchestrator/scripts/tests/test-dev-orchestrator.sh (T3/T5/T8) contrôle directement agent_lines ≤ 250 sur vf-dev-manager.md — seul check-agents.sh ne le vérifie pas (la phrase précédente était vraie au mot, trompeuse au sens)."
