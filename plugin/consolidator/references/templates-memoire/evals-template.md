<!--
  ...
  Source méthodo : plugin/reference/content/methodology/templates/memory/evals-template.md
  ...
  Rappel gouvernance proportionnée : en profil `leger`, EVALS n'est PAS posé à l'init — il est
  créé à la première éval réelle (EVAL-001).
-->
...
> Scalabilite : Index + Archive + Rotation aux jalons (meme logique que DECISIONS/LEARNINGS/BLOCKERS)
...
Le fichier commence toujours par un tableau index. L'agent lit l'index d'abord pour verifier les evaluations recentes et identifier les patterns de derive (colonne `#Ligne` + `offset`/`limit` pour cibler). Index maintenu par la machine : `bash .claude/scripts/reindex.sh --register=EVALS --apply` — ne jamais le rediger a la main. Methode, verdict et action vivent dans le body de chaque entree.
...
- Confrontation realite post-hoc a J+30 pour les decisions structurantes (DEC)

### Lien avec DECISIONS et LEARNINGS
- Une anomalie repetee (detectee dans 2+ evaluations) genere un LEARNING
- Une correction structurante issue d'une evaluation genere une DEC
- L'ID du LRN ou de la DEC associee est trace dans l'entree EVAL
...
### Rotation aux jalons
A chaque jalon majeur : passer en revue les evaluations, archiver les anciennes, consolider les patterns recurrents en LEARNINGS ou en DEC structurantes.
...
**Date** : [YYYY-MM-DD]
**Output evalue** : [path du fichier OU description explicite  -  ex : "DEC-017 prevision 50 ventes beta"]
...
- [ ] **Escalation**  -  decision structurante requise (creer DEC-XXX)

### Detail de l'action

[Description de ce qui a ete fait concretement : fichier modifie, nouveau template cree, DEC ouverte, etc.]
...
[DEC-XXX si l'evaluation declenche une decision structurante]
...
### Exemple 2  -  Confrontation realite sur DEC quantitative

**Contexte** : DEC-017 predisait 50 ventes beta en 6 semaines. A J+42, constater la realite.
...
**Contexte** : les templates CLAUDE.md et DECISIONS.md sont utilises depuis 6 mois. Sont-ils toujours pertinents ?
...
**Contexte** : plusieurs DEC ont ete signees en moins de 2 min de lecture sur les 30 derniers jours.

Methode : audit des signatures DEC. Constater le pattern. Cree une EVAL qui identifie le risque, declenche une DEC sur l'instauration d'un delai minimum de lecture (24h par exemple), et genere un LEARNING.
