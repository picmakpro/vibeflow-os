#!/usr/bin/env bash
# test-content-bundle.sh — Suite de vérification du module content-bundle (équipe content
...
#   T3  — check-agents.sh --strict (ADR-044) vert sur plugin/content-bundle/agents
#         (SKIP si le contrôleur du conductor est introuvable dans la disposition courante).
#   T4  — Le juge est read-only : content-clarity-judge sans Write ni Edit dans tools,
...
#         le repurposer refuse une pièce non validée ; le skill porte l'invariant.
#   T9  — Skill vf-content : description valide (déclencheur + portée) + aiguillage
#         geste simple vs mission (SEUIL_EQUIPE_CONTENT → vf-content-manager).
...
#   T12 — Rubric du juge : /100 explicite + seuil 80 + critère éliminatoire.
#
...
MANAGER="$MOD/agents/vf-content-manager.md"
JUDGE="$MOD/agents/content-clarity-judge.md"
WORKERS="vf-content-strategist vf-content-writer vf-content-repurposer"
TEAM="vf-content-manager vf-content-strategist vf-content-writer vf-content-repurposer content-clarity-judge"
SKILL="$MOD/skills/vf-content/SKILL.md"

echo "== test-content-bundle (module: $MOD) =="
...
if [ -f "$SKILL" ]; then
  n=$(wc -l < "$SKILL" | tr -d ' ')
  [ "${n:-999}" -le 500 ] || { ko "T2 densité : vf-content/SKILL.md = ${n}L (>500)"; t2_ok=0; }
else
...
if [ -z "$CHECK_AGENTS" ]; then
  ...
  if bash "$CHECK_AGENTS" --strict --agents-dir="$MOD/agents" >/dev/null 2>&1; then
    ok "T3 check-agents : --strict vert sur les agents de l'équipe content (ADR-044)"
  else
...
RP="$MOD/agents/vf-content-repurposer.md"
if [ -f "$RP" ]; then
  "$GREP" -qi 'REFUSE' "$RP" || { ko "T8 humain : le repurposer ne refuse pas explicitement une pièce non validée"; t8_ok=0; }
  "$GREP" -qi 'validation humaine' "$RP" || { ko "T8 humain : gate validation humaine absent du repurposer"; t8_ok=0; }
fi
if [ -f "$SKILL" ]; then
  "$GREP" -qi 'jamais' "$SKILL" && "$GREP" -qi 'validation humaine' "$SKILL" \
    || { ko "T8 humain : invariant validation humaine absent du skill"; t8_ok=0; }
fi
[ "$t8_ok" -eq 1 ] && ok "T8 humain : validation humaine non contournable (manager + repurposer + skill)"

# ---------------------------------------------------------------------------
# T9 — Skill vf-content : description + aiguillage
# ---------------------------------------------------------------------------
t9_ok=1
if [ -f "$SKILL" ]; then
  ...
  "$GREP" -q 'SEUIL_EQUIPE_CONTENT' "$SKILL" || { ko "T9 skill : seuil d'aiguillage absent"; t9_ok=0; }
  "$GREP" -q 'vf-content-manager' "$SKILL" || { ko "T9 skill : route mission → vf-content-manager absente"; t9_ok=0; }
else
...
# ---------------------------------------------------------------------------
# T12 — Rubric du juge : /100, seuil 80, critère éliminatoire
# ---------------------------------------------------------------------------
t12_ok=1
if [ -f "$JUDGE" ]; then
